<?php 
  session_start();
$isTouch = isset( $_SESSION["viewsubtaskss"] );
	if($isTouch==null){
		 echo '<script>window.location.href="index.php"</script>';
	}
	else{
       $viewsubtasks= $_SESSION["viewsubtaskss"];
...
<html class="no-js css-menubar" lang="en">
  <head>
    ...
    <title>รายละเอียดงานย่อย || PROJECT MANAGEMENT</title>
  ...
  <body class="animsition  page-project">
    ...
    <div class="page">
        ...
        <?php 
          include('Connnect.php');
          $sql = "SELECT * FROM task where TaskID = '".$viewsubtasks."'";
          $stmt = $conn->prepare($sql);
          $stmt->execute();
          while ($row = $stmt->fetch()) { 
        ?>
            <h1 class="page-title">รายละเอียดงานย่อย : <?php echo $row['TaskName']?></h1>
         <?php }?>
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="Project_Total.php">โครงการทั้งหมด</a></li>
              <li class="breadcrumb-item"><a href="Task_Total.php">งานทั้งหมด</a></li>
              <li class="breadcrumb-item"><a href="SubTask_Total.php">งานย่อยทั้งหมด</a></li>
              <li class="breadcrumb-item active">รายละเอียดงานย่อย</li>
            </ol>
          </div>
        ...
        <div class="row">
            <?php 
                ...
                $sql = "SELECT * FROM task AS  sub1
                        INNER JOIN  taskprojects AS  sub2 ON (sub1.TaskPoID = sub2.TaskPoID) where sub1.TaskID = '".$viewsubtasks."'";
                        $stmt = $conn->prepare($sql);
                        $stmt->execute();
                        while ($row = $stmt->fetch()) { 
            ?>
            ...
            <div class="card">
              <div class="card-block">
                <h4 class="card-title project-title">
                  ชื่องานย่อย : <?php echo $row['TaskName']; ?>
                </h4>
                <p class="card-text">
                  ชื่องาน : <?php echo $row['TaskPoName']; ?>
                </p>
              </div>
              <div class="card-block">
                <div class="row">
                    <div class="col-md-6">
                        <h4 class="project-option-title">ความคืบหน้างานย่อย</h4>
                        <div class="example mt-60">
                            <div class="pie-progress pie-progress-lg" data-plugin="pieProgress" data-barcolor="#4caf50"
                            data-size="200" data-barsize="8"  aria-valuenow="<?php echo $row['TaskProgress'];?>" role="progressbar">
                                <div class="pie-progress-content">
                                    <div class="pie-progress-number"><?php echo $row['TaskProgress'];?>%</div>
                                </div>
                            </div>
                        </div>
                    </div>
                         ...
                         <div class="card-block">
                            ...
                            <div class="pr-20 mt-5 text-left">
                                <span><h5>ความสำคัญงาน :
                                            ...
                                            <?php if($row['TaskPiority']==='สูง'){ ?>
                                                <span class="badge badge-danger piority-reds badge-lg"><b><?php echo $row['TaskPiority']; ?></b></span>
                                            <?php 	}  else if($row['TaskPiority']=="กลาง"){?>
                                                <span class="badge badge-warning piority-warnings badge-lg"><b><?php echo $row['TaskPiority']; ?></b></span>
                                            <?php 	} else if($row['TaskPiority']=="ต่ำ"){ 	?>
                                                <span class="badge badge-success piority-successs badge-lg"><b><?php echo $row['TaskPiority']; ?></b></span>
                                            <?php } ?>
                                            </span>
                                    </h5>
                                </span>
                            ...
                            <div class=" text-left">
                                <span><h5>สถานะงาน   : <span class="font-size-14 ml-10"><?php echo $row['TaskStatus']; ?></span></h5></span>
                            </div>
                            </div>
                               <div class="media mt-15 col-md-12">
                            <div class=" text-left">
                                <span><h5>น้ำหนักงาน   : <span class="font-size-14 ml-10"><?php echo $row['TaskEmphasis'],"%"; ?></span></h5></span>
                            </div>
                            </div>
                            <div class="media mt-10 col-md-12">
                            <div class="ml-3 text-left">
                                <span><h5>วันที่สร้าง : <a class="media-content  ml-10"><?php echo date("d-m-Y", strtotime($row['TaskCreateDate']));?> </a></h5><span>
                            </div>
                            </div>
                            <div class="media mt-10 col-md-12">
                            ...
                            <div class="media mt-10 col-md-12">
                            <div class="pr-20  text-left">
                                <span><h5>เริ่มวันที่ : <a class="media-content ml-15"><?php echo date("d-m-Y", strtotime($row['TaskStartDate']));?> </a></h5><span>
                            </div>
                            </div>
                            <div class="media col-md-12 mt-5">
                            <div class="text-left">
                                <span><h5>ถึงวันที่ : <a class="media-content ml-15"><?php echo date("d-m-Y", strtotime($row['TaskEndDate']));?> </a></h5><span>
                            </div>
                            </div>
                        </div>
                    </div>
              ...
              <div class="card-block ">
                ...
                <p class="card-text"> <?php echo $row['TaskDetail']; ?></p>
              </div>
              <div class="card-block ">
                              ...
                              <div data-role="content">
                                <ul class="list-group list-group-dividered list-group-full">
                                  <?php 
                                      ...
                                      WHERE log1.ActivitiesTaskPoID ='".$viewsubtasks."' ORDER BY log1.ActivitiesID DESC";
                                    ...
                                    <div class="media" style="padding:10px;">
                                      <div class="pr-20">
                                        <a class="avatar avatar-online" href="javascript:void(0)">
                                          <img src="assets/employee/<?php echo $rowac['EmployeePic']; ?>">
                                        </a>
                                      </div>
                                      <div class="media-body">
                  ...
                  <h4 class="project-option-title">หัวหน้างาน</h4>
                    <?php  
                        $sqlpm = "SELECT *  FROM assigntask AS d1 
                              ...
                              WHERE d1.TaskID ='".$viewsubtasks."' AND d1.AssignTaskPosition ='Task-SubManager'";
                ...
                <h4 class="project-option-title">สมาชิกงาน</h4>
                 <?php  
                    ...
                    <div class="row">
                     ...
                     <?php }
                       if($checkmmm==''){echo "<center>ไม่มีสมาชิกงาน</center>";}
                     ?>
              </div>
            </div>
          </div>
